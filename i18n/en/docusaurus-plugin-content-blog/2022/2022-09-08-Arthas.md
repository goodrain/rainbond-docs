...
Use SpringBoot as an example for this section.

First need to install the Rainbod cloud native application management platform, see the documentation [Installing Rainbond Allinone](https://www.rainbond.com/docs/installation/install-with-dind)
...
**2.3 Environmental Variables Configuration**
...
Use Arthas to diagnose the microservice Spring Cloud Pig, deployed on Rainbond and manage Arthas agent via Arthas Tunnel Unified Management.This subsection will use Spring Cloud Pig as an example.
...
Arthas is a very good Java diagnostic tool and is more complex to use in Kubernetes.Rainbond base is based on Kubernetes and abstracts the application model, places management applications in Kubernets, and allows users to use Arthas diagnostics more easily through Rainbond plugin mechanisms, lowering the threshold for using Arthas in Kubernetes and allowing users to focus only on business.
