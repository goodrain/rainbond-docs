...
- 根据 [软件和硬件环境要求](./resource-prepare/) 准备相关资源； 
...
后续操作过程中需要使用`kubectl`命令创建高可用存储等资源，请参考文档[kubectl命令行工具](/docs/v5.5/user-operations/tools/kubectl/)安装命令。
...
- ETCD对磁盘性能要求较高，所以请务必按照 [软件和硬件环境要求](./resource-prepare/) 准备相关资源，以免后续使用过程中出现不稳定情况；如果ETCD节点与其他属性节点复用，强烈建议存储使用SSD磁盘；
