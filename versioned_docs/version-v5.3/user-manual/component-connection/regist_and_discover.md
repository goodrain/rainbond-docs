...
1. **编辑依赖关系**：进入 _应用视图/应用拓扑_ 页面，点击“切换到编辑模式”将拓扑图切换到编辑模式，点击 A 组件的焦点连接到 B 组件，此时会弹出提示框提示你更新 A 组件。
...
- Rainbond 解决方案
  ...
  <img src="https://grstatic.oss-cn-shanghai.aliyuncs.com/docs/5.2/connection.png" title="组件间通信结构图"/>
...
> 根据上文描述的原理，如果依赖多个具有相同端口的组件，在当前组件网络空间下会存在端口冲突问题。解决这个问题的方式有两个：1. 所有组件的端口监听考虑读取环境变量 PORT，使组件监听端口可以被平台改变，然后在平台上为每个组件设置不同的监听端口。2. 如果都是 HTTP 服务，可以开通网络治理插件替换默认的插件，使用域名的方式来区分不同的组件，实现端口复用。参考 [组件之间网络治理之动态路由](../../get_start/best-practices/auto_router/)
...
1. [组件之间网络治理之动态路由](../../get_start/best-practices/auto_router/)
2. [组件之间网络治理之熔断](../../get_start/best-practices/circuit/)
