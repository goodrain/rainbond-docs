...
Rainbond内置组件创建有三种模式：[源码](../app-creation/service_create/#从源码创建)、[镜像](../app-creation/service_create/#从docker镜像创建)和[应用市场](../app-creation/service_create/#从应用市场安装)，它们分别具有不同的属性提供配置。
...
自动构建即通过一种方式自动触发Rainbond组件版本构建的操作，详细文档见[组件自动构建](./auto-deploy/)
