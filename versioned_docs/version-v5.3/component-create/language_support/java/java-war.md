...
1. 预编译处理会探测是否定义了启动命令配置文件 [Procfile](../../../component-create/language_support/procfile/) ,如果未定义会生成默认 War 包启动配置文件;

1. 预编译处理完成后,会根据语言类型选择 Java-war 的 buildpack 去编译项目.在编译过程中会安装定义的 JDK 版本,Web 服务;
...
关于 webapp-runner 详细配置请参考 [webapp-runner 使用说明](../../../component-create/language_support/java/webapp-runner/)
