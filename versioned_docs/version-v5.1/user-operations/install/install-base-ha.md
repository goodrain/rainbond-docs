...
在开始部署一个生产级的高可用Rainbond集群之前，强烈建议阅读 [Rainbond集群安装和运维原理解读](./install-d/)
...
> 初始化对接数据库参数说明 [初始化对接数据库](../tools/grctl/#初始化对接外部数据库)
...
- GlusterFS：
 ...
 [CentOS GlusterFS双机复制集群安装](../storage/centosgfs/)
  
 [Ubuntu GlusterFS双机复制集群安装](../storage/ubuntugfs/)

- 其它兼容NFS协议的共享存储

如果用户拥有可使用的兼容NFS协议的共享存储，可以直接使用。使 /grdata 目录在集群管理节点与计算节点间共享即可。

> Rainbond初始化对接存储参数说明 [初始化对接存储](../tools/grctl/#初始化对接存储)
...
[CentOS系统 vip配置](../backstage/centos_keepalived/)
[Ubuntu系统 vip配置](../backstage/ubuntu_keepalived)
