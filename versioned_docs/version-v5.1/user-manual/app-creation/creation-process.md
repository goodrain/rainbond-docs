...
* Step 2: 等待Rainbond对源代码进行检测，这个过程Rainbond将根据代码源信息获取源代码并进行语言类型、语言规范等检测，并从[rainbondfile](./language-support/etc/rainbondfile/)文件中读取组件属性。
...
* Step 5: 组件构建完成后即可通过端口绑定的默认域名进行访问。后续的管理维护流程参阅 [组件管理文档](../app-service-manage/service-properties/)
...
* Step 4: 构建完成后即可访问组件。后续的管理维护流程参阅 [组件管理文档](../app-service-manage/service-properties/)
