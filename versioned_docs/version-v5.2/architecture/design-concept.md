...
- 在开发阶段，对接代码仓库，自动识别 [开发语言类型](../component-create/language-support/)，不改变开发者习惯，尽量最大可能不修改现有代码，直接编译、构建和运行。

- 在架构阶段，如果已有系统没有分布式架构，Rainbond 提供 [Service Mesh](../advanced-scenarios/micro/service_mesh/) 架构，业务模块不改代码就能变成微服务架构。
