...
```bash
curl -o install.sh https://get.rainbond.com/install-dind.sh && bash ./install.sh
```
...
**1. [快速入门](/docs/quick-start/getting-started/) -> [部署你自己的应用](../use-manual/) -> [部署集群版 Rainbond](/docs/installation/install-with-ui/#从主机开始安装) -> [将应用从单机版迁移至集群部署](../ops-guide/migrate-app)**

在此路径中，你将从单机版开始起步。参考[快速入门](/docs/quick-start/getting-started/)可以快速部署起来示例应用，并体验到 Rainbond 应用管理功能。接下来你可以参考[用户使用手册](../use-manual/)部署你自己的应用，并直接在单机版 Rainbond 上开发和管理。后期你可以参考[从主机安装](/docs/installation/install-with-ui/#从主机开始安装)部署出集群版 Rainbond，并将单机版上的已有应用[迁移](../ops-guide/migrate-app)到你的集群进行管理。

**2. [快速入门](/docs/quick-start/getting-started/) -> [部署高可用集群](../installation/install-with-ui/ha) -> [在高可用集群中部署你的应用](../use-manual/)**

在此路径中，依然是从单机版开始起步。在你通过[快速入门](/docs/quick-start/getting-started/)体验到 Rainbond 管理上的便捷后，你可以直接参考文档[部署高可用集群](../installation/install-with-ui/ha)，并在高可用集群中[部署管理你的应用](../use-manual/)。
...
**单节点体验版本安装过程中如果长时间未完成，那么请参考文档 [单机体验版本安装问题排查指南](../troubleshooting/installation/dind) 进行故障排查。或加入 [微信群](/community/support#微信群)、[钉钉群](/community/support#钉钉群) 寻求帮助。**
