...
description: Rainbond 和 KubeSphere 的区别在于，前者更偏应用交付与低门槛上手，后者更偏围绕 Kubernetes 建设完整平台能力，适合不同团队阶段。
hide_title: true
...
<Head>
  ...
  <meta
    ...
    content="Rainbond 和 KubeSphere 的区别在于，前者更偏应用交付与低门槛上手，后者更偏围绕 Kubernetes 建设完整平台能力，适合不同团队阶段。"
  />
...
<CompareHeroGraphic
  title="Rainbond 和 KubeSphere 区别是什么？"
  subtitle="如果你更关注低学习门槛、应用交付和环境复制，Rainbond 更适合；如果你更关注围绕 Kubernetes 建设更完整的平台能力，KubeSphere 更合适。"
...
| --- | --- | --- |
| 产品定位 | 不用懂 Kubernetes 的开源容器平台 | 以 Kubernetes 为核心的平台能力扩展 |
...
<CompareInlineCta
  title="如果你已经更偏向 Rainbond，下一步不要停在对比页"
  description="这类团队最容易卡在“看懂了，但还没真正开始”。更稳的动作是先去安装决策页，确认自己应该走试装还是正式安装。"
  primaryLabel="先选安装路径"
...
## 最后
如果你当前更想先把业务应用稳定交付出去，优先看 Rainbond；如果你更想先建设完整平台能力，优先看 KubeSphere。
<CompareCtaFooter
  installLink={{
    href: '/install-hub',
    label: '先选安装路径',
    note: '先判断自己是试装、正式部署还是已有 Kubernetes，再进入对应安装路径。',
  }}
  caseLink={{
    href: '/blog/case-csg-app',
