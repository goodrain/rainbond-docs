...
Rainbond 是 AI 应用运行平台。它将 AI 生成的项目、AI 开源软件和业务应用，以容器方式运行在自己的服务器或 Kubernetes 上，并持续完成部署、运维、升级与交付。
...
Rainbond 不是单独的大模型平台，也不是单独的 Agent 平台，而是 AI 应用运行平台。RainAgent、RainSkills 和大模型服务等能力共同覆盖 AI 应用的部署、排障、运维与持续交付。
