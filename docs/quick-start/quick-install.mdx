...
```bash
curl -o install-rainbond.sh https://get.rainbond.com/install-rainbond.sh && bash ./install-rainbond.sh
```
...
默认添加了 `10000～10010` 10个端口供业务的 TCP 端口使用。如果你需要更多的 TCP 端口，通过脚本中打印的命令，删除容器重新启动并添加 `-p` 映射新的端口。
