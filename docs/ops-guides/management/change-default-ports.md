...
如果修改了 API 端口（默认8443），则需要通过浏览器 `http://192.168.1.1:7070` (默认7070) 登录控制台，进入 **平台管理 -> 集群 -> 编辑**，修改 `API 通信地址` 中的端口为新的端口。

```bash title="kubectl edit apisixroute -n rbd-system rbd-api-api"
spec:
  stream:
  - backend:
      ...
    match:
      ingressPort: 7443 # 修改为新的访问端口,  默认8443
    ...
``` 
