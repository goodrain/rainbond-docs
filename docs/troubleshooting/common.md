...
3. **本地 curl 测试**  
   ...
   curl localhost:80
   ```
   若返回正常页面内容，说明服务已启动并监听端口。若无响应或报错，需检查服务启动日志，确认服务是否正常启动、端口配置是否正确。
4. **逐步向外排查**  
   从容器内 curl 容器 IP，再从集群内其他节点 curl 服务 ClusterIP，最后从外部 curl 域名或 NodePort。
5. **查看组件和网关日志**  
   检查组件日志，确认服务是否报错或异常退出；检查网关日志，确认请求是否到达网关及后端服务。

:::tip
如果业务容器镜像中没有 `curl`、`telnet`、`netstat` 或 `ss` 等排障工具，可以在组件 **Web终端** 中点击容器旁的工具图标，打开 toolbox 调试容器进行网络、端口和 DNS 排查。详细说明请参考 [Web 终端](/docs/how-to-guides/app-ops/web-terminal)。
:::

## 5. 无法上传离线包、软件包、Jar、WAR、ZIP等

通常是因为本地浏览器与 Rainbond WebSocket 通信失败导致的。你可以按以下顺序检查：

1. 如果使用 v6.9.2 及以后版本，且默认集群的 console 和 region 都在 `rbd-system` 命名空间中，控制台会通过 `rbd-api-websocket` Service 代理访问集群端 WebSocket。此时浏览器侧访问控制台的 7070 端口需要支持 WebSocket 升级。
2. 检查访问 Rainbond 控制台的反向代理、负载均衡或网关是否放通 WebSocket Upgrade 请求头。
3. 如果是独立集群、多集群或外部 region 场景，在 `平台管理 -> 集群 -> 编辑集群` 中确认 `WebSocket` 地址是否正确，并确认浏览器可以访问该地址。

## 6. Web 终端无法使用

Web 终端无法使用，通常是因为 WebSocket 通信失败导致的。排查方式与上传离线包、软件包等场景一致：

1. v6.9.2 及以后版本的默认集群场景，优先确认控制台 7070 端口是否支持 WebSocket 代理。
2. 如果控制台前面还有 Nginx、Ingress、负载均衡或安全设备，需要确认它们没有拦截 WebSocket Upgrade。
3. 独立集群、多集群或外部 region 场景，继续在 `平台管理 -> 集群 -> 编辑集群` 中检查 `WebSocket` 地址。
4. 如果 Web 终端可以打开但默认容器缺少排障命令，请使用 [Web 终端](/docs/how-to-guides/app-ops/web-terminal) 中的 toolbox 调试容器。
...
3. 超出 K8S NodePort 端口范围，请[扩展端口范围](../ops-guides/management/change-tcp-range)
