---
title: 概述
description: Rainbond Yaml&Helm 指南概述
keywords:
- Yaml Helm 指南概述
- Rainbond Yaml Helm
---
...
在原生 Kubernetes 中没有概念与 Rainbond 中的应用概念对应，但用户可以指定将 Yaml 中所有的资源定义都部署到 Rainbond 中的应用中去。当用户在 Yaml 中定义了非 Workload 类的资源（如 Service 、Sercet 等）时，Rainbond 会将其转化后保存在 `应用 > k8s资源` 列表中，并提供编辑入口供用户后续管理。
...
Rainbond 会从用户所提供 Yaml 定义的所有资源中抽取 Workload 相关的定义，并在应用中转化生成 Rainbond 可管理的组件实例。转化的过程中，Rainbond 会自动识别所有可以被管理的规格定义(Spec)，将不同的属性，交由扩展后的 RAM 模型管理，大部分属性延续了 Rainbond 一贯的易用使用体验，而另一部分，则交由 `其他设置 > Kubernetes属性` 页面进行管理。


## 使用手册

学习通过 Yaml 部署应用的使用手册参考 [yaml文件识别创建](/docs/use-manual/component-create/package-support/yaml)。

学习通过 Helm 部署应用的使用手册参考 [基于 Helm 命令部署 Helm 应用](/docs/use-manual/component-create/helm-support/helm-cmd-install)
...
[使用 Yaml 部署 Wordpress 和 Mysql](/docs/yaml-helm-guide/yaml-example/)

[使用 Helm 部署 Wordpress 和 Mysql](/docs/yaml-helm-guide/helm-example/)
