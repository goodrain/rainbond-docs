...
如果使用的是默认 CNB + Paketo 启动方式，运行时通常还会自动注入一组 `JAVA_TOOL_OPTIONS`。常见内容类似：
...
| 默认参数 | 说明 |
|----------|------|
| `-Djava.security.properties=.../java-security.properties` | 加载 Paketo 提供的额外 Java 安全配置 |
...
| `-Dorg.springframework.cloud.bindings.boot.enable=true` | 开启 Spring Cloud Bindings 自动绑定支持 |
