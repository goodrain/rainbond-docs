...
最常用的是 `JAVA_TOOL_OPTIONS`，适合直接追加 JVM 参数或 Spring Boot 的 `-D` 参数。

示例：

```bash
JAVA_TOOL_OPTIONS=-Dserver.port=8082 -XX:+UseG1GC
```

### JVM 内存计算相关

Paketo 会自动计算 JVM 内存参数。如果你需要微调内存计算，可以使用以下变量：

| 环境变量 | 说明 |
|----------|------|
| `BPL_JVM_HEAD_ROOM` | 为非堆内存预留额外空间，单位为百分比；未设置时由 Paketo 自动估算 |
| `BPL_JVM_THREAD_COUNT` | 调整运行时线程数估算；未设置时使用 Paketo 默认估算 |
| `BPL_JVM_LOADED_CLASS_COUNT` | 调整已加载类数量估算；未设置时使用 Paketo 默认估算 |
| `BPL_JVM_CLASS_ADJUSTMENT` | 对类数量估算做额外修正；未设置时按默认计算处理 |
