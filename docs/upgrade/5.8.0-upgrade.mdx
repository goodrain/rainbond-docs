...
```bash
helm upgrade --set Component.rbd_app_ui.image=registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.8.0-release-allinone --set operator.image.tag=v5.8.0-release --set Cluster.installVersion=v5.8.0-release rainbond rainbond/rainbond-cluster -n rbd-system
```
