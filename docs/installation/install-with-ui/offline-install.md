...
这一步你需要提前准备好存放离线包的目录，下面的 /rainbond 可修改成你想存放的目录地址。

```bash
export RAINBOND_INSTALL_PATH=~/rainbond
wget -P ${RAINBOND_INSTALL_PATH} https://rainbond-pkg.oss-cn-shanghai.aliyuncs.com/offline/5.X/rainbond-offline-v5.8.0-release.tgz
```
...
```bash
source ${RAINBOND_INSTALL_PATH}/offline/install_docker_offline.sh 
```
