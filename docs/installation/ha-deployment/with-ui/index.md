...
- 根据 [软件和硬件环境要求](/docs/installation/ha-deployment) 准备相关资源； 
...
若还没有高可用数据库可根据[参考文档](/docs/ops-guide/Install-extension/mysql-ha)部署MySQL主从复制。
...
后续操作过程中需要使用`kubectl`命令创建高可用存储等资源，请参考文档[kubectl命令行工具](/docs/ops-guide/tools/kubectl/)安装命令。
...
部署请参考文档 [GlusterFS分布式存储](/docs/ops-guide/storage/deploy-glusterfs/) 。

-  其它兼容NFS协议的共享存储

如果用户拥有可使用的其他兼容NFS协议的共享存储，例如阿里云NAS存储，NFS存储(高可用环境不推荐)，Rainbond也可对接使用。

具体请参考文档  [对接阿里云NAS](/docs/ops-guide/cluster-manage/init-region-storage/#对接阿里云NAS) 。
...
[CentOS keepalived配置](/docs/ops-guide/install-extension/centos_keepalived/) 或 [Ubuntu keepalived配置](/docs/ops-guide/install-extension/ubuntu_keepalived/)。
...
- ETCD对磁盘性能要求较高，所以请务必按照 [软件和硬件环境要求](/docs/installation/ha-deployment/ha-deployment) 准备相关资源，以免后续使用过程中出现不稳定情况；如果ETCD节点与其他属性节点复用，强烈建议存储使用SSD磁盘；
...
部署完成后将 All-In-One 模式部署的控制台迁移至 Rainbond 中管理，使其具有高可用特性  参考文档[控制台迁移](/docs/installation/ha-deployment/with-ui/console-recover)。
