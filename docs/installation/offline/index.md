...
VERSION=${VERSION:-'v5.13.0-release'}
...
### 导入镜像包
```bash
docker load -i rainbond-v5.13.0-release.tar
```
...
```bash
tar xvf rainbond-cluster-5.13.0.tgz
```
...
# 重新打 tag 并推送镜像
docker tag registry.cn-hangzhou.aliyuncs.com/goodrain/builder:v5.13.0-release goodrain.me/builder:latest
docker tag registry.cn-hangzhou.aliyuncs.com/goodrain/runner:v5.13.0-release goodrain.me/runner:latest
