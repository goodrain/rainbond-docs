...
wget https://rainbond-pkg.oss-cn-shanghai.aliyuncs.com/offline/5.X/rainbond-offline-v5.11.0-amd64.tar
docker pull registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-dind-allinone
vim Dockerfile
```
...
```file
FROM registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-dind-allinone
COPY rainbond-offline-v5.11.0-amd64.tar /app/offline.tar
```

 - 保存后继续执行以下命令

```shell
docker build -t registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-dind-allinone .
docker save -o rainbond-v5.11.0-dind-allinone.tar registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-dind-allinone
```

- 导入 tar 包到你需要离线安装的服务器中。

```shell
docker load -i rainbond-v5.11.0-dind-allinone.tar
```
...
-e EIP=$EIP \
registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-dind-allinone \
...
```shell
mkdir -p ~/build_offline && cd ~/build_offline
wget https://rainbond-pkg.oss-cn-shanghai.aliyuncs.com/offline/5.X/rainbond-offline-v5.11.0-arm64.tar
docker pull registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-arm64-dind-allinone
vim Dockerfile
```
...
```file
FROM registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-arm64-dind-allinone
COPY rainbond-offline-v5.11.0-arm64.tar /app/offline.tar
```

 - 保存后继续执行以下命令

```shell
docker build -t registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-arm64-dind-allinone .
docker save -o rainbond-v5.11.0-arm64-dind-allinone.tar registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-arm64-dind-allinone
```

- 导入 tar 包到目标服务器中。

```shell
docker load -i rainbond-v5.11.0-arm64-dind-allinone.tar
```
...
-v /opt/rainbond:/opt/rainbond \
-e EIP=$EIP \
registry.cn-hangzhou.aliyuncs.com/goodrain/rainbond:v5.11.0-arm64-dind-allinone \
&& docker logs -f rainbond-allinone
```
...
nerdctl -n k8s.io tag registry.cn-hangzhou.aliyuncs.com/goodrain/runner:v5.11.0-release goodrain.me/runner:latest
nerdctl -n k8s.io tag registry.cn-hangzhou.aliyuncs.com/goodrain/builder:v5.11.0-release goodrain.me/builder:latest
```
