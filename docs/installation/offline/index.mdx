...
<h2> 4. load离线环境所需镜像 </h2>

load tar 包

```bash
docker exec -ti rainbond-allinone bash
nerdctl -n k8s.io load < offline.tar
```

查看当前版本 `registry.cn-hangzhou.aliyuncs.com/goodrain/resource-proxy` 的基础镜像。

```bash
nerdctl -n k8s.io images|grep registry.cn-hangzhou.aliyuncs.com/goodrain/resource-proxy
```

切换 rbd-resource-proxy 的基础镜像，将 `image:` 后面的内容修改为你上面查询到的版本。
