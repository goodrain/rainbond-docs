...
本文描述如何通过不用懂 Kubernetes 的开源容器平台 [Rainbond](https://www.rainbond.com/?channel=emqx) 一键安装高可用 [EMQX](https://www.emqx.com) 集群。这种方式适合不太了解 Kubernetes、容器化等复杂技术的用户使用，降低了在 Kubernetes 中部署 EMQX 的门槛。
...
[Rainbond](https://www.rainbond.com/?channel=emqx) 是一款不用懂 Kubernetes 的开源容器平台。借助于它，用户可以在图形化界面中完成微服务的部署与运维。借助 Kubernetes 和容器化技术的能力，将故障自愈、弹性伸缩等自动化运维能力赋能给用户的业务。
...
- 部署好的 Rainbond ：例如 [快速体验版本](https://www.rainbond.com/docs/quick-start/quick-install/?channel=emqx)，可以在个人 PC 环境中以启动一个容器的代价运行。
- 可以连接到互联网。
