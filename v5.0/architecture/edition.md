...
本文档列举Rainbond具备的重点基础功能，随着版本升级本文档列举不一定完全，如有疑问请于社区留言咨询。
...
- 通过.NetCore源码创建，支持自定义.NetCore编译和运行环境
- 通过Helm Chart源码创建（5.1版本计划)

### Docker镜像持续构建
- DockerCompose部署，支持DockerCompose配置文件导入批量创建服务组件
- 支持DockerRun格式直接运行镜像，通过dockerRun命令格式直接创建服务组件
...
### 支持Windows应用创建
- 支持基于Dockerfile,镜像创建Windows服务（.net或mfc架构传统应用）
...
- 支持集群虚拟IP资源池管理（5.1版本计划）
- 支持暴露域名、服务的访问情况实时监控数据（5.1版本计划）
- 支持插件化扩展应用网关业务功能（安全控制体系）（5.1版本计划）
