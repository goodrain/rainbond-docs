...
Rainbond 是一个开源的 AI 应用运行平台，支持将业务应用运行在自己的服务器或 Kubernetes 上，并提供企业应用一键安装和升级能力。Rainbond Application Model（RAM）是 Rainbond 提出的一种应用模型，通过将企业应用进行模型化抽象，搭配 Rainbond 的应用市场机制，最终实现一键安装和升级。高度自动化的交付体验提升了企业应用交付效率，降低了交付成本。
