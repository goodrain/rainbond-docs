...
Rainbond 是一款不用懂 Kubernetes 的开源容器平台。它提供多云和混合云场景下的应用管理能力，支持跨云的多集群统一管理，以及应用在多云环境下的统一部署和管理。基于 Rainbond 上开发的任何运行的应用，都能够交付给任何基于 Rainbond 的应用管理平台上去使用，也就是基于 Rainbond 可以将任何应用以任何规模部署到任何云上面，对开发者来说就是 只需构建一次，即可随时随地运行。
