...
function Card({Name, children, githubUrl, blogUrl}) {
  return (
      <div className="card">
        ...
        <div className="card__footer">
          <div className="button-group button-group--block">
            <a className="button button--secondary" href={ githubUrl }>Github</a>
            { blogUrl ? <a className="button button--secondary" href={ blogUrl }>Blog</a> : '' }
          </div>
        </div>
      </div>
  );
}
export function TocCardList(): JSX.Element {
    return (
...
export function ContributorCardList(): JSX.Element {
        ...
        <div className="row">
          ...
          contributors.map(item => (
            <div className="col col--2" style={{ marginTop: '1rem'}}>
              <a href={item}>
                < img className={`${ styles.avatarPhoto } avatar__photo`} src={`${item}.png`} />
              </a>
            </div>
          ))}
        </div>
