...
- <b>连接超时时间</b>: 网关代理连接后端组件的连接阶段超时时间。
- <b>请求超时时间</b>: 网关请求后端组件的请求超时时间。
- <b>响应超时时间</b>: 网关请求后端组件响应的超时时间。
- <b>上传大小限制</b>: POST 请求的最大请求 Body，当请求报错 413 时，请将此值增大。
- <b>Websocket 支持</b>: 开启后自动填充 Websocket 相关的代理请求头。
- <b>ProxyBuffer 设置</b>: 是否开启 ProxyBuffer 设置。
- <b>自定义请求头</b>: 可以自定义网关到组件的请求头信息，默认会透传所有用户请求头。
