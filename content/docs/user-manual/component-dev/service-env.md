...
| 变量名                | 变量值                | 说明                                                                                                              |
|-----------------------|-----------------------|:------------------------------------------------------------------------------------------------------------------|
| ES_SELECTNODE         | 选择宿主机的 ID       | 用于定点调度，指定调度到某台宿主机上，节点的 ID 通过 grctl node list 可得. 注意该环境变量不能和`本地存储`同时使用 |
| ES_SELECTHOST         | 选择宿主机的 HOSTNAME | 用于定点调度，根据 HOSTNAME 调度到某台宿主机上，节点的 HOSTNAME 通过 grctl node list 可得                                   |
