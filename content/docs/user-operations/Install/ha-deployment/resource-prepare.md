...
|属性类别|属性|概述|服务器数量|
| ---- | ---- | ---- | :----: |
|Kubernetes|管理节点|Kubernetes 管理节点与 Rainbond 管理服务混合部署|至少2台|
|Kubernetes|计算节点|为工作负载提供计算资源|至少2台|
|Kubernetes|ETCD节点|用于保存Kubernetes集群所有的网络配置和对象的状态信息|至少3台|
|Rainbond|网关节点|云上应用访问的入口|至少2台|
|Rainbond|构建节点|Rainbond平台执行源码构建任务节点|至少2台|
|Rainbond|数据库节点|在集群外搭建数据库，供平台存储元数据|至少2台|
|Rainbond|存储节点|部署开源版本 GlusterFS 集群，为平台提供共享存储|至少3台|
