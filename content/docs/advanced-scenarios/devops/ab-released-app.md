...
* 不同的服务
  ...
  > 一般是指不同应用的不同版本，在Rainbond平台是不同的组件。
...
通过 [应用网关](/docs/user-manual/gateway/gateway/) -> [访问策略](/docs/user-manual/gateway/traffic-control/) 添加以下两个HTTP访问策略：
...
两个内部服务本质上是同一个业务，使用了相同的服务端口，若在默认情况下有端口冲突是不能同时依赖它们的，这时我们需要先为`外部服务` 开通工作在7层的网络治理插件（平台默认提供），插件的工作原理将复用80端口，通过不同的域名等HTTP元素实现高级路由来选择使用的下级依赖服务；因此，要使用此插件的前提是：
...
>
> 我们通常建议将通信地址和端口读取环境变量的方式，只需要在内部服务上设置 [连接信息](/docs/user-manual/app-service-manage/service-rely/#服务连接信息管理) 变量即可。

* ` 外部服务` 依赖 `内部服务2` ，操作方式见 [添加服务依赖](/docs/user-manual/app-service-manage/service-rely/#服务依赖管理) 或直接 [操作拓扑图添加](/docs/user-manual/app-manage/app-topology/#编辑模式)
