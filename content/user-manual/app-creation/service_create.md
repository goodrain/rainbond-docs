...
应用的定义中已经提到, 应用是由各个不同的服务组件构成, 那么应用的创建就离不开服务组件的创建. 服务组件创建的方式有3种, 分别是: 源码创建, 从Docker镜像创建, 从 [应用市场](/user-manual/app-store/) 安装.
...
- <a href="../language-support/java/" target="_blank">Java源码创建应用</a>
- <a href="../language-support/php/" target="_blank">PHP源码创建应用</a>
- <a href="../language-support/python/" target="_blank">Python源码创建应用</a>
- <a href="../language-support/nodejs/" target="_blank">Node.js源码创建应用</a>
- <a href="../language-support/golang/" target="_blank">Golang源码创建应用</a>
- <a href="../language-support/netcore/" target="_blank">.Net源码创建应用</a>
- <a href="../language-support/html/" target="_blank">Html静态源码创建应用</a>
- <a href="../language-support/dockerfile/" target="_blank">Dockerfile源码创建应用</a>
...
Rainbond 的底层是 Kubernetes(k8s), k8s 是一个用于自动化部署，扩展和管理容器化应用的系统. 而在 k8s 容器化应用的过程中, Docker镜像发挥着十分重要的作用. 有了镜像, 我们就可以做到"一次构建, 到处运行(Build once, Run anywhere)".

你可以将项目的代码, 运行时环境, 依赖库, 配置等资源或信息通过 Dockerfile 打包成一个或多个镜像, 上传到Docker Hub, 或quay.io, 又或是自己的私有仓库. 对于项目中所需要的数据库, 消息队列等中间件, 或者Elastic Search, Kibana, Grafana等日志收集工具, 你甚至可以直接在 Docker Hub 找别人制作好的镜像, 不需要自己去制作. 然后, 你可以通过镜像的名字, Docker Run 命令, 或者 Docker Compose, 将的镜像跑在 Rainbond 上。
...
命令行工具类软件镜像: Docker, Homebrew Bottles
{{% /notice %}}
...
Docker Run 命令时, Rainbond 会解析出 Docker Run 命令中的镜像, 端口, 环境变量，内存和存储等定义信息, 然后生成Rainbond应用（服务）抽象, 最后再被调度到 Kubernetes 上运行起来。

> Rainbond 支关心镜像, 端口, 环境变量，内存和持久化挂载等信息, 其它信息将会被忽略。此方式支持大多数Dockerhub上提供的镜像运行方式执行。
